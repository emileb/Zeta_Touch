...
import com.opentouchgaming.androidcore.*
...
open class EngineOptionsEDuke32Kt(prefix: String, val userFilesDir: String) : EngineOptionsInterface {
    ...
    override fun showDialog(activity: Activity, engine: GameEngine, version: Int, update: Function<Int, Void>?) {
        ...
        binding.deleteCfgButton.setOnClickListener {
            ...
            showAlert(
                activity, "Delete config file", "Delete EDuke32 config files?\n"
            ) {
    ...
    fun selectVersion(activity: Activity, version: Int) {
        ...
        val items = arrayOf<Pair<String, View?>>(
            Pair("GLES 2.0", binding.gl2Options),
            Pair("Software", binding.swOptions)
        )

        // Create and Setup the renderer spinner options
        SpinnerWidget(
            activity, binding.rendererSpinner.root,
            "Rendering backend", "Select rendering backend used. Will affect performance and features",
            items, RENDERER_PREFIX, RENDERER_DEFAULT,
        ...
        SpinnerWidget(
            activity, binding.cacheSpinner.root,
            "Cache memory", "Size of Cache memory to allocate",
            cacheItems, CACHE_SIZE_PREFIX, CACHE_SIZE_DEFAULT,
        ...
        SwitchWidget(
            activity, binding.loadAutoloadSwitch.root,
            "Load 'autoload' folder", "Automatically load files in the 'autoload' folder",
            AUTOLOAD_PREFIX, AUTOLOAD_DEFAULT,
            0
        )
    }
