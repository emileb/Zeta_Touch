...
import com.opentouchgaming.androidcore.ui.widgets.*
...
class EngineOptionsRaze : EngineOptionsInterface {
    ...
    override fun showDialog(activity: Activity, engine: GameEngine, version: Int, update: Function<Int, Void>?) {
        ...
        binding.deleteCfgButton.setOnClickListener {
            ...
            showAlert(
                activity, "Delete config file", "Delete Raze config file?\n(${file})"
            ) {
                File(file).delete()
                File(file2).delete()
            }
    ...
    fun selectVersion(activity: Activity, version: Int) {
        ...
        val items = arrayOf<Pair<String, View?>>(
            Pair("GLES 2.0+", binding.modernGl2OptionsLayout),
            Pair("GLES 3.2", null)
        )

        // Create and Setup the backend spinner options
        SpinnerWidget(
            activity, binding.modernRendererSpinner.root,
            "Rendering backend", "Select rendering backend used. Will affect performance and features.",
            items, MODERN_RENDERER_PREFIX, MODERN_RENDERER_DEFAULT,
            R.drawable.setting_gpu
        )

        SwitchWidget(
            activity, binding.usePolymostSwitch.root,
            "Use Polymost renderer",
            "Use the original Polymost renderer, disable to use the Raze3D renderer",
            USE_POLYMOST_PREFIX, USE_POLYMOST_DEFAULT
        )

        // GLES 2.0
        SwitchWidget(
            activity, binding.modernUseMappedBuffersSwitch.root,
            "Use mapped GL buffer",
            "Change vertex buffer loading. Can change performance so test it on/off.",
            MODERN_MAP_BUFFER_PREFIX, MODERN_MAP_BUFFER_DEFAULT
        )

        SwitchWidget(
            activity, binding.modernForceGlslV100Switch.root,
            "Force GLES2 shader compliance", "Enable if crash on new game. For very old devices.",
            MODERN_FORCE_V100_PREFIX, MODERN_FORCE_V100_DEFAULT
        )

        SwitchWidget(
            activity, binding.modernGlslPrecisionMedSwitch.root,
            "Use 'medium' GLSL precision", "May increase performance. Disable if geometry appears to shake or wobble.",
            MODERN_MEDIUM_GLSL_PREFIX, MODERN_MEDIUM_GLSL_DEFAULT
        )
    ...
    override fun getRunInfo(version: Int): EngineOptionsInterface.RunInfo {
        ...
        if (SwitchWidget.fetchValue(AppInfo.getContext(), USE_POLYMOST_PREFIX, USE_POLYMOST_DEFAULT))
            args += " +set vid_renderer 0 " // Polymost
        else
            args += " +set vid_renderer 1 " // New renderer

        if (SwitchWidget.fetchValue(AppInfo.getContext(), MODERN_MEDIUM_GLSL_PREFIX, MODERN_MEDIUM_GLSL_DEFAULT))
            args += " +set gles_glsl_precision 1 "

        if (SwitchWidget.fetchValue(AppInfo.getContext(), MODERN_MAP_BUFFER_PREFIX, MODERN_MAP_BUFFER_DEFAULT))
            args += " +set gles_use_mapped_buffer 1 "

        if (SwitchWidget.fetchValue(AppInfo.getContext(), MODERN_FORCE_V100_PREFIX, MODERN_FORCE_V100_DEFAULT))
            args += " +set gles_force_glsl_v100 1 "
    ...
    override fun launchMultiplayer(
        activity: Activity?, engine: GameEngine?, version: Int, mainArgs: String?,
